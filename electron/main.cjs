...
function fieldOpsReport(snapshot, engagement) {
  ...
  const permit=engagement.permit,operator=permit?.attestation?.operator,approver=permit?.approvingAuthority
...
app.whenReady().then(async () => {
  ...
  trustAuthority = new TrustAuthority(app.getPath('userData'), { safeStorage })
  await trustAuthority.initialize()
  engagementStore = new EngagementStore(app.getPath('userData'), { entitlement: () => licenseManager.snapshot(), trust: trustAuthority })
