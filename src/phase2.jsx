...
const scenarioData = { ...legacyScenarioData, ...additionalMissionScenarios }
...
export function LabSimulation({ mission, onExit, onComplete }) {
  ...
  useEffect(() => {
    if (startRef.current || !rangeApi || !scenario) return
    ...
    rangeApi.availability().then(availability => {
      ...
      setEngine('live')
      setHistory([{ type: 'system', lines: ['DAEMONCORE MISSION OS RANGE // 6.0', `MODE ${mode.toUpperCase()} // RUN ${result.session.seed}`, result.session.caseVariant ? `CASE VARIANT // ${result.session.caseVariant.label.toUpperCase()} // ${result.session.caseVariant.brief}` : 'ADAPTIVE CASE // STANDARD EVIDENCE CONTRACT', 'CONTAINMENT VERIFIED // INTERNAL NETWORK // ZERO HOST MOUNTS // EGRESS DENIED', '', scenario.intro, '', `This is a disposable shell inside ${result.manifest.operatorContainer}.`, 'Prove outcomes with evidence. Type help for the interface contract.'] }])
    }).catch(error => {
  ...
  useEffect(() => { if (!busy && ['live', 'simulation'].includes(engine)) inputRef.current?.focus() }, [busy, engine, history])
  ...
  const run = async raw => {
    ...
    if (matched?.objective === mission.objectives.length-1 && done.filter(objective => objective < matched.objective).length < matched.objective) {
      push({ type: 'command', text: raw }, { type: 'system', lines: ['SUBMISSION BLOCKED // EVIDENCE THRESHOLD NOT MET', 'Complete and preserve every preceding objective before submitting a conclusion.'] })
      return
    }
  ...
  const debrief = completion?.debrief || { overall: Math.max(70, 100-hints*15), dimensions:[{id:'evidence',label:'Evidence coverage',score:100},{id:'independence',label:'Operator independence',score:Math.max(40,100-hints*22)},{id:'method',label:'Method discipline',score:100},{id:'tempo',label:'Time discipline',score:Math.max(50,100-Math.floor(seconds/300)*5)}], strengths:['Evidence coverage'], remediation:hints?'Repeat without guidance and preserve the same evidence threshold.':'Advance to a live Docker run or a higher-autonomy mode.', nextAction:'Continue through the active Mission OS route.' }
  const formatTime = s => `${String(Math.floor(s / 60)).padStart(2, '0')}:${String(s % 60).padStart(2, '0')}`
  const leaveRange = async () => { if (rangeApi && ['live', 'provisioning'].includes(engine)) await rangeApi.stop().catch(() => {}); onExit() }
  const finishRange = async () => { if (rangeApi && engine === 'live') await rangeApi.stop().catch(() => {}); onComplete({ mission, score, hints:completion?.hints??hints, seconds:completion?.seconds??seconds, mode:completion?.mode||mode, seed:completion?.seed||session?.seed, evidenceDigest:completion?.evidenceDigest, debrief, caseVariant:completion?.caseVariant||session?.caseVariant, receipt:completion?.receipt||receipt, launchReceipt:receipt }) }
  const exportReceipt = async () => { const result=await rangeApi?.exportReceipt(); if(result&&!result.canceled)setReceiptExported(true) }

  if (engine === 'probing' || engine === 'provisioning') return <div className="range-gate"><div className="range-gate-grid"/><div className="range-gate-core"><div className="range-loader"><Hexagon size={78}/><i/></div><span>PHASE 10 // RANGE ORCHESTRATOR</span><h1>{engine === 'probing' ? 'Checking the engine.' : 'Building the sealed range.'}</h1><p>{engine === 'probing' ? 'Locating Docker and verifying the local runtime.' : 'Pulling the operator image, building the target, and proving containment before shell access is released.'}</p><div className="boot-sequence"><span className="done"><Check/> Scenario manifest</span><span className={engine === 'provisioning' ? 'active' : ''}><Activity/> Container network</span><span><ShieldCheck/> Containment proof</span><span><Terminal/> Root shell</span></div><button onClick={leaveRange}>Cancel launch</button></div></div>

  if (engine === 'offline') return <div className="range-gate offline"><div className="range-gate-grid"/><div className="range-gate-core"><div className="offline-mark"><HardDrive size={37}/></div><span>RANGE ENGINE OFFLINE</span><h1>Docker isn’t ready.</h1><p>{engineError} Live Academy missions use Docker to enforce the sealed network and disposable target boundary.</p><div className="offline-requirements"><div><CheckCircle2/><span>Docker Engine 24+ or Docker Desktop</span></div><div><CheckCircle2/><span>Linux containers</span></div><div><CheckCircle2/><span>2 GB free memory</span></div></div><div className="gate-actions"><button className="sim-primary" onClick={() => window.open('https://docs.docker.com/get-docker/')}>Install Docker <ArrowRight size={15}/></button><button onClick={() => setEngine('simulation')}>Use simulation fallback</button><button onClick={onExit}>Return to range</button></div></div></div>

  if (report) return <div className="simulation-shell completion-screen">
    <div className="completion-grid" />
    <div className="completion-core"><div className="completion-seal"><ShieldCheck size={46}/></div><span>MISSION COMPLETE // EVIDENCE ACCEPTED</span><h1>{mission.title}</h1><p>You maintained the declared boundary and converted observations into a defensible conclusion.</p><div className="completion-score"><div><small>OPERATION SCORE</small><strong>{score}</strong></div><div><small>READINESS SIGNAL</small><strong>{debrief.overall}%</strong></div><div><small>GUIDANCE USED</small><strong>{hints}</strong></div><div><small>ELAPSED</small><strong>{formatTime(seconds)}</strong></div></div><section className="after-action"><header><span>AFTER-ACTION REVIEW</span><strong>{completion?.caseVariant?.label||session?.caseVariant?.label||'STANDARD CASE'}</strong></header>{debrief.dimensions.map(dimension=><div key={dimension.id}><span>{dimension.label}</span><div><i style={{width:`${dimension.score}%`}}/></div><strong>{dimension.score}</strong></div>)}<p><b>NEXT ACTION</b>{debrief.nextAction}</p><p><b>REMEDIATION</b>{debrief.remediation}</p></section>{receipt&&<div className="receipt-seal"><Fingerprint size={16}/><span>DIGEST-SEALED LAUNCH RECEIPT</span><code>{receipt.digest}</code></div>}<div className="earned-xp"><Zap size={18}/> +{score} EXPERIENCE LOGGED</div>{receipt&&<button className="ghost" onClick={exportReceipt}>{receiptExported?'RECEIPT EXPORTED':'EXPORT INTEGRITY RECEIPT'}</button>}<button className="sim-primary" onClick={finishRange}>Destroy range &amp; return <ArrowRight size={16}/></button></div>
  </div>

  return <div className="simulation-shell">
    <header className="simulation-header"><div><button onClick={leaveRange}><ArrowLeft size={16}/></button><div className="sim-brand"><Hexagon size={26}/><i/></div><div><span>DAEMONCORE // {engine === 'live' ? 'SEALED RANGE TERMINAL' : 'SIMULATION FALLBACK'}</span><strong>{mission.title}</strong></div></div><div className="sim-telemetry"><span><i/> {engine === 'live' ? 'CONTAINMENT VERIFIED' : 'ISOLATED'}</span><span><Clock3 size={13}/>{formatTime(seconds)}</span><span>{receipt ? `RECEIPT ${receipt.digest.slice(0,12)}` : containment ? `${containment.network} // EGRESS BLOCKED` : `MISSION // ${mission.id.toUpperCase()}`}</span><button onClick={leaveRange}>DESTROY &amp; EXIT</button></div></header>
    <div className="simulation-body">
      <section className="sim-console"><div className="console-toolbar"><div><span/><span/><span/></div><strong>{engine === 'live' ? `ROOT@DC-${mission.id.toUpperCase()}` : 'DC_RANGE_SIMULATOR'}</strong><span>{busy ? 'PROCESS RUNNING…' : engine === 'live' ? 'INTERNAL NETWORK // NO EGRESS' : 'LOCAL FALLBACK'}</span></div><div className="terminal-history" ref={scrollRef}>{history.map((item, i) => <TerminalLine key={i} item={item}/>)}</div><form className="terminal-input" onSubmit={e => { e.preventDefault(); run(input) }}><span>{engine === 'live' ? `root@dc-${mission.id}` : 'operator@dc-range'}</span><em>›</em><input ref={inputRef} autoFocus readOnly={busy} aria-busy={busy} value={input} onChange={e => setInput(e.target.value)} spellCheck="false" autoComplete="off" placeholder={engine === 'live' ? 'run anything inside the sealed range' : 'enter a simulation command'}/><button disabled={busy}>{busy ? 'RUNNING' : 'EXECUTE'}</button></form></section>
      <aside className="sim-sidebar"><div className="range-orientation"><Terminal/><div><span>LIVE COMMAND DESTINATION</span><strong>{engine === 'live' ? `root@dc-${mission.id}` : 'DaemonCore simulation terminal'}</strong><p>Commands entered in the terminal beside this panel run only in the current disposable training environment.</p><small>NEXT // {done.length ? mission.objectives[done.length] || 'Review accepted evidence' : mode === 'guided' ? 'Type help to view the guided runbook' : mission.objectives[0]}</small></div></div><div className="sim-target"><span>TARGET CONTEXT</span><div><Crosshair size={19}/><strong>{scenario.target}</strong></div><small>{scenario.subnet}</small></div><div className="objective-panel"><div className="sim-panel-title"><Flag size={14}/><span>MISSION OBJECTIVES</span><strong>{done.length}/{mission.objectives.length}</strong></div>{mission.objectives.map((objective, i) => <div className={`sim-objective ${done.includes(i) ? 'done' : ''}`} key={objective}><span>{done.includes(i) ? <Check size={12}/> : `0${i + 1}`}</span><p>{objective}</p></div>)}</div><div className="evidence-panel"><div className="sim-panel-title"><FileText size={14}/><span>EVIDENCE LOCKER</span><strong>{evidence.length}</strong></div>{evidence.length ? evidence.map((item, i) => <div className="evidence-item" key={item}><span>E-{String(i + 1).padStart(2, '0')}</span><p>{item}</p></div>) : <div className="empty-evidence"><Database size={20}/><p>No evidence collected</p></div>}</div><button className="hint-button" onClick={() => run('hint')}><Sparkles size={15}/> Request guidance <span>-75 PTS</span></button></aside>
    </div>
  </div>
}
...
export function LessonPlayer({ lesson, onExit, onComplete }) {
  ...
  const application = course.code === 'WEB-201'
    ? { page: 'webforge', label: 'Web Forge' }
    : course.code.endsWith('-301')
      ? { page: 'enterprise', label: 'Enterprise Forge' }
      : { page: 'labs', label: 'Lab Range' }
  ...
  return <div className="lesson-overlay"><aside className="lesson-rail"><div className="lesson-brand"><Hexagon/><span>DC // ACADEMY</span></div><button onClick={onExit}><ArrowLeft size={15}/> Exit lesson</button><div className="lesson-designation"><span>{course.code} // {content.level||'FOUNDATION'}</span><h3>{lesson.title}</h3><small>{content.minutes} MIN // PRACTICAL + VALIDATION</small></div><nav>{content.sections.map((s, i) => <button key={s.title} className={`${section === i ? 'active' : ''} ${section > i ? 'complete' : ''}`} onClick={() => !complete && setSection(i)}><span>{section > i ? <Check size={11}/> : `0${i + 1}`}</span><div><small>SECTION</small><strong>{s.title}</strong></div></button>)}<button disabled={!practicalPassed} className={`${atCheck ? 'active' : ''} ${complete ? 'complete' : ''}`} onClick={() => practicalPassed&&setSection(content.sections.length)}><span>{complete ? <Check size={11}/> : String(content.sections.length+1).padStart(2,'0')}</span><div><small>{practicalPassed?'VALIDATION':'WORKBENCH LOCK'}</small><strong>Knowledge check</strong></div></button></nav><div className="lesson-rail-progress"><span>LESSON PROGRESS</span><div><i style={{ width: `${Math.min(100, section / content.sections.length * 100)}%` }}/></div><strong>{Math.round(Math.min(100, section / content.sections.length * 100))}%</strong></div></aside>
    <main className="lesson-content"><header><span>TACTICAL INSTRUCTION // {course.code}</span><div className="lesson-workflow"><b className="active">LEARN</b><i/><b>PRACTICE</b><i/><b>VALIDATE</b><i/><b>APPLY</b></div><div><Clock3 size={14}/> {content.minutes} MIN</div></header>{!atCheck ? <article><div className="lesson-kicker">SECTION // 0{section + 1} &nbsp;·&nbsp; {content.environment}</div><h1>{content.sections[section].title}</h1><p className="lesson-lead">{content.sections[section].body}</p><section className="lesson-context"><BookOpen/><div><span>CURRENT ACTIVITY // INSTRUCTION</span><strong>Read and analyze inside this lesson.</strong><p>Command blocks below are worked examples. This page does not execute them, and you should not paste them into your Windows terminal.</p></div><em>NO LIVE TARGET</em></section>{section===0&&<><section className="lesson-outcome"><Target/><div><span>OPERATOR OUTCOME</span><strong>{content.outcome}</strong></div></section><div className="lesson-brief-grid"><section><span>YOU WILL BE ABLE TO</span>{content.objectives?.map(item=><p key={item}><CheckCircle2/>{item}</p>)}</section><section><span>BEFORE YOU START</span>{content.prerequisites?.map(item=><p key={item}><ChevronRight/>{item}</p>)}</section></div></>}{practical&&<section className="workshop-block"><header><div><span>GUIDED ANALYSIS // STEP {String(section+1).padStart(2,'0')}</span><h2>{practical.title}</h2></div><Code2/></header><p>{practical.instruction}</p><div className="command-boundary"><ShieldCheck/><div><span>REFERENCE COMMAND // NOT RUN HERE</span><p>Study how this command supports the method. Complete the in-app workbench for credit. A live terminal appears only after you separately launch {application.label}.</p></div></div><div className="command-block reference"><div><span>WORKED EXAMPLE // {content.environment}</span><button onClick={()=>copyCommand(practical.command)}>{copied===practical.command?<Check/>:<Copy/>}{copied===practical.command?'COPIED':'COPY EXAMPLE'}</button></div><pre>{practical.command}</pre></div><div className="expected-block"><span>WHAT THIS EXAMPLE WOULD SHOW</span><pre>{practical.expected}</pre></div><div className="analysis-block"><Search/><div><span>READ THE RESULT</span><p>{practical.analysis}</p></div></div></section>}{lastSection&&content.exercise&&<section className="operator-exercise"><header><Flag/><div><span>OPERATOR EXERCISE // IN APP</span><h2>Produce the artifact.</h2></div></header><p>{content.exercise.brief}</p><div><span>DELIVERABLE</span><strong>{content.exercise.deliverable}</strong></div><ul>{content.exercise.success.map(item=><li key={item}><CheckCircle2/>{item}</li>)}</ul></section>}{lastSection&&<InteractiveWorkbench scenario={content.interactive} onPassed={setPracticalScore}/>} {lastSection&&<section className="application-handoff"><Terminal/><div><span>FIELD APPLICATION // AFTER VALIDATION</span><strong>Your live practice is launched separately in {application.label}.</strong><p>Finish this lesson first. The completion screen can take you directly there without implying that lesson examples executed.</p></div></section>}{lastSection&&content.references?.length>0&&<section className="lesson-references"><span>PRIMARY REFERENCES</span>{content.references.map(reference=><a key={reference.url} href={reference.url} target="_blank" rel="noreferrer">{reference.label}<ArrowRight/></a>)}</section>}<div className="operator-note"><div><UserRound/></div><div><span>OPERATOR RULE</span><p>Preserve the command, the raw signal, and what the signal does not prove.</p></div></div><button className="lesson-next" disabled={lastSection&&!practicalPassed} onClick={next}>{lastSection ? practicalPassed?`Knowledge check unlocked // ${practicalScore}%`:'Pass the in-app operator workbench to continue' : 'Continue instruction'} <ArrowRight size={16}/></button></article> : <article className="lesson-check">{complete ? <div className="lesson-complete"><div><Award size={44}/></div><span>LESSON MASTERED</span><h1>Operator standard met.</h1><p>The workshop, {practicalScore}% practical score, and validation are now written to your operator record.</p><div><strong>+180 XP</strong><small>PRACTICAL + KNOWLEDGE VALIDATED</small></div><div className="lesson-complete-actions"><button className="lesson-secondary" onClick={() => onComplete({...lesson,practicalScore})}>Return to pathway</button><button className="lesson-next" onClick={() => onComplete({...lesson,practicalScore,nextPage:application.page})}>Apply in {application.label} <ArrowRight size={16}/></button></div></div> : <><div className="lesson-kicker">KNOWLEDGE VALIDATION // PRACTICAL {practicalScore}%</div><h1>{content.check.q}</h1><div className="lesson-options">{content.check.options.map((option, i) => <button key={option} className={`${selected === i ? 'selected' : ''} ${selected !== null && i === content.check.answer ? 'correct' : ''} ${selected === i && !correct ? 'wrong' : ''}`} onClick={() => setSelected(i)}><span>{String.fromCharCode(65 + i)}</span>{option}{selected !== null && i === content.check.answer && <Check size={16}/>}</button>)}</div>{selected !== null && <div className={`answer-rationale ${correct ? 'correct' : ''}`}>{correct ? <CheckCircle2/> : <AlertTriangle/>}<div><strong>{correct ? 'Correct signal.' : 'Recalibrate.'}</strong><p>{content.check.rationale}</p></div></div>}<button className="lesson-next" disabled={!correct} onClick={next}>Complete lesson <ArrowRight size={16}/></button></>}</article>}</main>
  </div>
}
...
export function PhaseBadge({ complete, live }) {
  ...
  return <span className="phase-live"><Activity size={11}/> {live ? 'LIVE SEALED RANGE' : 'SIMULATION'}</span>
}
...
export function RangeEngineCard() {
  ...
  return <div className={`range-engine-card ${ready ? 'ready' : status.state}`}><div className="engine-mark"><Terminal size={19}/><i/></div><div><span>PHASE 10 RANGE ENGINE</span><strong>{status.state === 'checking' ? 'PROBING LOCAL RUNTIME' : ready ? `DOCKER ${status.version || ''} // READY` : status.state === 'preview' ? 'WEB PREVIEW // SIMULATION PATH' : 'DOCKER ENGINE REQUIRED'}</strong><small>{ready ? 'Containment is verified again before every shell opens.' : status.reason || 'Install and start Docker to unlock the unrestricted sealed shell.'}</small></div><div className="engine-policies"><span><ShieldCheck/> INTERNAL NETWORK</span><span><HardDrive/> ZERO HOST MOUNTS</span><span><LockKeyhole/> AUTO-TEARDOWN</span></div><em>{ready ? 'READY' : status.state === 'checking' ? 'CHECKING' : 'OFFLINE'}</em></div>
}
