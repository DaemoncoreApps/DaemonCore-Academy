...
export function LabSimulation({ mission, onExit, onComplete }) {
  ...
  if (engine === 'offline') return <div className="range-gate offline"><div className="range-gate-grid"/><div className="range-gate-core"><div className="offline-mark"><HardDrive size={37}/></div><span>RANGE ENGINE OFFLINE</span><h1>Docker isn’t ready.</h1><p>{engineError} Phase 3 uses Docker to enforce the sealed network and disposable target boundary.</p><div className="offline-requirements"><div><CheckCircle2/><span>Docker Engine 24+ or Docker Desktop</span></div><div><CheckCircle2/><span>Linux containers</span></div><div><CheckCircle2/><span>2 GB free memory</span></div></div><div className="gate-actions"><button className="sim-primary" onClick={() => window.open('https://docs.docker.com/get-docker/')}>Install Docker <ArrowRight size={15}/></button><button onClick={() => setEngine('simulation')}>Use simulation fallback</button><button onClick={onExit}>Return to range</button></div></div></div>
