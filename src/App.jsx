...
function AcademyPage({ selectModule, profile }) {
  ...
  return <div className="page academy-page">
    <div className="page-intro"><div><span className="section-code">COMPLETE PAID COURSE // VERSIONED CONTENT</span><h2>Network recon,<br/><em>without the bullshit.</em></h2></div><p>Eight written lessons, eight validation checks, four drills, and a real sealed range. Every progress number below comes from your operator record.</p></div>
...
function DrillsPage({ startQuiz, profile }) {
  ...
  return <div className="page drills-page"><div className="drill-hero"><div><span className="section-code">REPEATABLE SKILL CONDITIONING</span><h2>Pressure makes<br/><em>patterns visible.</em></h2><p>Four complete drill sets. Scores below are calculated only from your attempts.</p><button className="primary" onClick={()=>startQuiz(drillSets[0])}><Swords size={17}/> Start protocol drill</button></div><div className="score-orbit"><div><small>ACCURACY</small><strong>{accuracy}</strong><span>{attempts.length?'FROM OPERATOR RECORD':'NO ATTEMPTS YET'}</span></div></div></div>
    ...
    <div className="drill-list">{drillSets.map((d,i)=>{const Icon=icons[i],best=bestFor(d.id);return <button key={d.id} onClick={()=>startQuiz(d)}><span className="drill-num">0{i+1}</span><div className="drill-icon"><Icon size={20}/></div><div><strong>{d.title}</strong><p>{d.description}</p></div><span><Clock3 size={13}/>3 QUESTIONS</span><span>PB // {best?`${best}%`:'—'}</span><Play size={17}/></button>})}</div>
  </div>
}
