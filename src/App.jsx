...
const emptyData = { schemaVersion:3, profile:{handle:null,createdAt:null,xp:0,level:1,streak:0,bestStreak:0,lastActiveDate:null,weekKey:null,weeklyMinutes:0,weeklyGoalMinutes:180,completedMissions:[],completedLessons:[],completedWebLabs:[],lessonAttempts:[],missionAttempts:[],webLabAttempts:[],drillAttempts:[],capstoneAttempts:[],achievements:[],activity:[]},settings:{reduceMotion:false,compactMode:false,uiScale:1.25} }
...
function AcademyPage({ selectModule, profile }) {
  ...
  const progress=progressFor(course),webProgress=progressFor(webCourse),totalMinutes=course.estimatedMinutes+webCourse.estimatedMinutes,totalLessons=course.lessons.length+webCourse.lessons.length
  return <div className="page academy-page">
    <div className="page-intro"><div><span className="section-code">FULL-SPECTRUM ACADEMY // VERSIONED CONTENT</span><h2>Learn the system.<br/><em>Test the evidence.</em></h2></div><p>{totalLessons} practical lessons, {Math.floor(totalMinutes/60)} hours {totalMinutes%60} minutes of guided workshops, eight drills, six core ranges, and twenty-two Web Forge conditions. Every progress number below comes from your operator record.</p></div>
    <div className="academy-summary"><div><ShieldCheck size={25}/><div><span>ACADEMY LIBRARY</span><strong>TWO COMPLETE PATHWAYS</strong></div></div><div className="summary-progress"><span>CORE COURSE PROGRESS</span><div><i style={{width:`${progress}%`}}/></div><strong>{progress}%</strong></div><div className="cert-meta"><span>CONTENT LENGTH</span><strong>{Math.floor(totalMinutes/60)}H {totalMinutes%60}M</strong></div></div>
    ...
    <div className="module-list"><button className="module-card active" onClick={() => selectModule({...course,icon:Radar,progress})}>
      <div className="module-index">01</div><div className="module-icon" style={{'--accent':'#ff3038'}}><Radar size={25}/></div><div className="module-copy"><span>{course.code} // {progress===100?'MASTERED':progress?'IN PROGRESS':'READY'}</span><h3>{course.title}</h3><p>{course.description}</p><div className="module-tags"><em><BookOpen size={13}/>{course.lessons.length} lessons</em><em><Clock3 size={13}/>{course.estimatedMinutes} min</em></div></div>
      <div className="module-progress"><Ring value={progress} size={62}/><span>{progress===100?'MASTERED':progress?'IN PROGRESS':'BEGIN'}</span></div><ChevronRight className="module-arrow" size={20}/>
    </button><button className="module-card active" onClick={() => selectModule({...webCourse,icon:Braces,progress:webProgress})}>
      <div className="module-index">02</div><div className="module-icon" style={{'--accent':'#ff3038'}}><Braces size={25}/></div><div className="module-copy"><span>{webCourse.code} // {webProgress===100?'MASTERED':webProgress?'IN PROGRESS':'READY'}</span><h3>{webCourse.title}</h3><p>{webCourse.description}</p><div className="module-tags"><em><BookOpen size={13}/>{webCourse.lessons.length} lessons</em><em><Clock3 size={13}/>{webCourse.estimatedMinutes} min</em></div></div>
      <div className="module-progress"><Ring value={webProgress} size={62}/><span>{webProgress===100?'MASTERED':webProgress?'IN PROGRESS':'BEGIN'}</span></div><ChevronRight className="module-arrow" size={20}/>
    </button></div>
  </div>
}
