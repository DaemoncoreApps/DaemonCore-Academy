...
function ModuleDetail({ module, onBack, startLesson, profile }) {
  ...
  return <div className="page detail-page"><button className="back-button" onClick={onBack}><ArrowLeft size={16}/> Course library</button><div className="detail-hero"><div className="detail-icon"><Icon size={38}/></div><div><span>{module.code} // COMPLETE COURSE</span><h2>{module.title}</h2><p>{module.description}</p><div className="module-tags"><em><BookOpen size={13}/>{module.lessons.length} lessons</em><em><Clock3 size={13}/>{module.estimatedMinutes} min</em><em><Zap size={13}/>{module.lessons.length*180} XP</em></div></div><div className="detail-progress"><Ring value={progress} size={90} stroke={5}/><span>COURSE PROGRESS</span></div></div>
    <div className="detail-layout"><section><div className="section-title"><div><span>CURRICULUM</span><h3>Operational sequence</h3></div></div><div className="lesson-list">{course.lessons.map((item,i)=>{const complete=profile.completedLessons.includes(item.id),active=!complete&&course.lessons.findIndex(l=>!profile.completedLessons.includes(l.id))===i; return <button key={item.id} className={active?'active':''} onClick={()=>startLesson(item)}><span>{complete?<Check size={14}/>:String(i+1).padStart(2,'0')}</span><div><small>LESSON // {complete?'COMPLETE':active?'UP NEXT':'READY'}</small><strong>{item.title}</strong></div><em>{item.minutes} MIN</em>{active?<Play size={16} fill="currentColor"/>:<ChevronRight size={16}/>}</button>})}</div></section><aside className="path-aside"><div className="mentor-card"><div className="mentor-avatar"><ShieldCheck/></div><span>COURSE STANDARD</span><h3>EVIDENCE OVER VIBES</h3><p>Every lesson ends with a validation check. Progress is written only after the correct answer.</p></div><button className="primary full" onClick={()=>startLesson(course.lessons.find(l=>!profile.completedLessons.includes(l.id))||course.lessons[0])}><Play size={16}/> {progress===100?'Review from start':'Resume course'}</button><div className="unlock-card"><Target size={20}/><div><span>FIELD APPLICATION</span><strong>The Ghost Port</strong><small>Available in Lab Range</small></div></div></aside></div>
  </div>
}
...
export default function App() {
  ...
  return <div className="app-shell"><Sidebar page={page} setPage={p=>{setPage(p);setModule(null)}} collapsed={navigationCollapsed} setCollapsed={setCollapsed} profile={operator}/><main><Topbar title={title} profile={operator}/>{current}<footer className="app-footer"><span>DAEMONCORE ACADEMY // PHASE 5</span><span><i/> LICENSED LOCAL-FIRST PLATFORM</span><span>{licensing.license.tierLabel?.toUpperCase()||'COMMERCIAL CORE READY'}</span></footer></main>{mission&&<MissionModal mission={mission} onClose={()=>setMission(null)} onLaunch={()=>{setActiveMission(mission);setMission(null)}}/>}{quiz&&<QuizModal drill={quiz} onClose={()=>setQuiz(null)} onComplete={completeQuiz}/>} {toast&&<Toast message={toast}/>}</div>
}
