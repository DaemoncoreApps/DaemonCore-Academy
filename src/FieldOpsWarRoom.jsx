...
import { Activity, BadgeCheck, ChevronRight, Crosshair, Database, FileText, Fingerprint, Plus, Radar, ShieldCheck, Timer, TriangleAlert, Zap } from 'lucide-react'
...
export function FieldOpsWarRoom({ data, identity, engagement, onNewEngagement, onWorkspace: changeWorkspace }){
  ...
  const modules=[
    ...
    {id:'campaigns',number:'02',verb:'ORCHESTRATE',title:'Campaigns',detail:activeCampaigns?'Operation currently active':'Run repeatable assessment plans',icon:Zap},
    {id:'evidence',number:'03',verb:'PRESERVE',title:'Evidence vault',detail:captures.length?`${captures.length} sealed captures`:'Awaiting first capture',icon:Database},
    {id:'findings',number:'04',verb:'REPORT',title:'Findings',detail:openFindings?`${openFindings} require disposition`:'Register is clear',icon:FileText},
    {id:'chaos',number:'05',verb:'VALIDATE',title:'Chaos Engine',detail:'Run bounded resilience trials',icon:Activity},
  ]
