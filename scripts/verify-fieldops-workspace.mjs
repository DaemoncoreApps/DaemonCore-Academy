...
try{
  ...
  assert.deepEqual(surface.result.hardCaps,{ports:30,webServices:4,tlsHandshakes:4,redirects:0})
  assert.equal(surface.result.web[0].tls.protocol,'TLSv1.3')
  assert.equal(surface.result.comparison.status,'baseline-established')
  store.portSurvey=async()=>({tested:2,hardCap:30,observations:[{port:80,state:'open',latencyMs:9},{port:443,state:'open',latencyMs:12}]})
