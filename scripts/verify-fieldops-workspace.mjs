...
try{
  const store=new EngagementStore(directory,{entitlement:()=>({fieldOps:true}),now:()=>current,lookup:async()=>[{address:'93.184.216.34',family:4}]})
  ...
  assert.equal(persisted.captures.length,2)
  console.log('FieldOps workspace verified // sealed captures, findings, dispositions, retests, and persistence')
