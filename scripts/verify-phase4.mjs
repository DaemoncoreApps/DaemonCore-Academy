...
const {EngagementStore,isPublicAddress}=require('../electron/engagement-store.cjs')
...
try{
  ...
  assert.equal(state.engagements.length,1);assert.deepEqual(state.engagements[0].targets,['example.com']);assert.deepEqual(state.engagements[0].ports,[80,443])
  ...
  console.log('Phase 4 verified // licensing, offline grace, scope boundary, and public-target enforcement')
