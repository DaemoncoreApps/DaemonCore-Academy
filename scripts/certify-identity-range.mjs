...
try{
  ...
  await execute('LDAP delegated edge',`ldapsearch -LLL -N -Q -Y GSSAPI -H ldap://dc01.daemoncore.lab -b 'DC=daemoncore,DC=lab' '(&(objectClass=group)(cn=Backup Operators Lab))' cn member`,/Backup Operators Lab[\s\S]*svc\.backup/i)
